...
func AdminGetSubscription(c *fiber.Ctx) error {
...
func AdminGetSubscriptionByUserId(c *fiber.Ctx) error {
	...
	subscription_id := c.Params("subscription_id")

	subscriptionID, err := strconv.ParseInt(subscription_id, 10, 64)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "invalid id",
		})
